...
yinc="106"
padding="107"
cropregion="${bw}x${bh}+0+0"
tcropregion="${text_width}x${text_height}+72+30"
rc=0
input="a4page_0.png"
output="a4-processed.png"
paper_source="a4page.png"
extra_arg=""
extra_args="-depth 300"
page_height="3500"
page_width="2490"
draw_paper="true"
pageno=0
while read bfile; do
  ...
  echo $code
  convert "${input}" \
    \( ${bfile} -depth 95 -crop ${cropregion} ${extra_arg} \) -geometry +${x}+${y} -composite \
    \( ${bfile} -depth 300 -crop ${tcropregion} ${extra_arg} \) -geometry +${tx}+${ty} -composite \
    ${extra_args} "${output}"
  mv "${output}" "${input}"
...
#  mv "${output}" "${input}"
  x=$((x+xinc))
  tx=$((tx+xinc))
  if [ $((x+bw+padding)) -gt ${page_width} ]; then
    x=$padding
    y=$((y+yinc))
    tx=$((x+bw/2-text_width/2))
    ty=$((y+bh))
