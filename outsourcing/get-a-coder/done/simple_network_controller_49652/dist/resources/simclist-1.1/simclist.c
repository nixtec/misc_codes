...
static inline struct list_entry_s *list_findpos(const list_t *l, unsigned int posstart);

/* list initialization */
int list_init(list_t *l) {
    if (l == NULL) return -1;

    l->head = l->mid = l->tail = NULL;
    l->numels = 0;
    l->iter_active = 0;
    l->iter_pos = 0;
    l->iter_curentry = NULL;
    l->spareels = malloc(SIMCLIST_MAX_SPARE_ELEMS * sizeof(struct list_entry_s *));
    l->spareelsnum = 0;
...
static void *list_get_minmax(const list_t *l, int versus) {
    ...
    curminmax = l->head->data;
    for (s = l->head->next; s != NULL; s = s->next) {
...
static inline struct list_entry_s *list_findpos(const list_t *l, unsigned int posstart) {
    ...
    if (posstart >= l->numels) return NULL;

    x = (float)(posstart+1) / l->numels;
    if (x <= 0.25) {
        /* first quarter: get to posstart from head */
        for (i = 0, ptr = l->head; i < posstart; ptr = ptr->next, i++);
    } else if (x < 0.5) {
        ...
        for (i = l->numels-1, ptr = l->tail; i > posstart; ptr = ptr->prev, i--);
    }
...
int list_insert_at(list_t *l, const void *data, unsigned int pos) {
    ...
    if (l->spareelsnum > 0) {
        ...
        lent = malloc(sizeof(struct list_entry_s));
        if (lent == NULL)
            return -1;
    }
    ...
    if (l->numels == 0) {       /* list was empty */
        lent->next = lent->prev = NULL;
        l->head = l->mid = l->tail = lent;
        l->numels++;
        return 1;
    }

    if (pos == 0) {             /* list not empty, but this is the head */
        lent->next = l->head;
        lent->prev = NULL;
        l->head->prev = lent;
        l->head = lent;
        if (! (l->numels % 2)) l->mid = l->mid->prev;   /* now even */
        l->numels++;
        return 1;
    }

    /* elem to be appended in the middle (or tail) of the list */
...
int list_delete_range(list_t *l, unsigned int posstart, unsigned int posend) {
    ...
    if (l->iter_active) return -2;
    if (posend < posstart) return -1;
    if (posend >= l->numels) return -1;
    ...
    assert(posstart == 0 || lastvalid != NULL);
    i = posstart;
    if (l->attrs.copy_data) {
    ...
    assert(i == posend+1 && (posend != l->numels || tmp == NULL));

    if (posstart == 0) {    /* head gone */
        l->head = tmp;
    } else {    /* first != NULL */
        lastvalid->next = tmp;
    }
    if (posend == l->numels-1) { /* tail gone */
        l->tail = lastvalid;
    } else {    /* tmp != NULL */
        tmp->prev = lastvalid;
    }
...
int list_clear(list_t *l) {
    ...
    if (l->attrs.copy_data) {
        /* also free user data */
        for (i = 0, s = l->head; i < l->numels; i++) {
            s = s->next;
            if (l->head->data != NULL) free(l->head->data);
            if (l->spareelsnum < SIMCLIST_MAX_SPARE_ELEMS) {
                l->spareels[l->spareelsnum++] = l->head;
            } else {
                free(l->head);
            }
            l->head = s;
...
int list_concat(const list_t *l1, const list_t *l2, list_t *dest) {
    ...
    el = dest->head = malloc(sizeof(struct list_entry_s));
    el->prev = NULL;
    for (cnt = 0, srcel = l1->head; cnt < l1->numels; cnt++) {
        el->data = srcel->data;
        el->next = malloc(sizeof(struct list_entry_s));
        el->next->prev = el;
        el = el->next;
        srcel = srcel->next;
    }
    dest->mid = el;     /* approximate position (adjust later) */
    for (cnt = 0, srcel = l2->head; cnt < l2->numels; cnt++) {
        el->data = srcel->data;
        el->next = malloc(sizeof(struct list_entry_s));
        el->next->prev = el;
        el = el->next;
    ...
    } else if ((err/2)-1 < 0) { /* correct pos LEFT (err/2)-1 moves */
        err = -(err/2-1);
        for (cnt = 0; cnt < (unsigned int)err; cnt++) dest->mid = dest->mid->prev;
    }
 
    assert(list_repOk(dest));
...
int list_sort(list_t *l, int versus) {
    ...
    list_sort_quicksort(l, versus, 0, l->head, l->numels-1, l->tail);
    assert(list_repOk(l));
    return 0;
}
...
static void list_sort_quicksort(list_t *l, int versus, 
    ...
    if (pivotid > 0) {
        ...
        if (l->threadcount < SIMCLIST_MAXTHREADS-1) {
            struct list_sort_wrappedparams *wp = malloc(sizeof(struct list_sort_wrappedparams));
            l->threadcount++;
            traised = 1;
            wp->l = l;
            wp->versus = versus;
...
int list_comparator_string(const void *a, const void *b) { return strcmp(b, a); }
...
size_t list_meter_string(const void *el) { return strlen(el); }
...
static int list_repOk(const list_t *l) {
    ...
    ok = (l != NULL) && (
            /* no elems, head & tail --> NULL */
            (l->numels != 0 || (l->head == NULL && l->mid == NULL && l->tail == NULL))
            /* 1 elem => head == tail both !NULL */
            && (l->numels != 1 || (l->head != NULL && l->mid == l->head && l->tail == l->head))
         );
    
    if (!ok) return 0;

    if (l->numels >= 1) {
        /* head and tail terminating endpoints */
        ok = (l->head->prev == NULL && l->tail->next == NULL);
        if (!ok) return 0;
        /* correct referencing */
        for (i = 0, s = l->head; i < (l->numels-1)/2 && s->next != NULL; i++, s = s->next) {
            if (s->next->prev != s) break;
        }
        ok = (i == (l->numels-1)/2 && l->mid == s);
        if (!ok) return 0;
