...
static int comm_man_sendfile(comm_man_t *c, int sockfd, const char *file)
{
  ...
  sprintf(tmpname, "%s/temp/%s", c->local_host_name, buf);
  sprintf(realname, "%s/%s", c->local_host_name, file);
...
int comm_man_init(const char *ip, const char *user, const char *pass)
{
  ...
  ret = mkdir(ip, 0777);
  if (ret == -1) {
    if (errno != EEXIST) { /* we allow pre-existing directories */
      perror(cman.local_host_name);
      return -1;
    }
  }
  sprintf(buf, "%s/temp", ip);
  ret = mkdir(buf, 0777);
  if (ret == -1) {
    if (errno != EEXIST) { /* we allow pre-existing directories */
      perror(buf);
    ...
    const struct sockaddr *address, socklen_t address_len)
{
  ...
  sprintf(c->sock_table[sockfd]->sockname_cs, "%s.%hd.cs.fifo",
      address->sin_addr, address->sin_port);
  mkfifo(c->sock_table[sockfd]->sockname_cs, 0666);
  sprintf(c->sock_table[sockfd]->sockname_sc, "%s.%hd.sc.fifo",
      address->sin_addr, address->sin_port);
    ...
    const struct sockaddr *address, socklen_t address_len)
{
  ...
  sprintf(c->sock_table[sockfd]->sockname_cs,
      "%s.%d.cs.fifo", address->sin_addr, address->sin_port);
  ...
  sprintf(c->sock_table[sockfd]->sockname_sc,
      "%s.%d.sc.fifo", address->sin_addr, address->sin_port);
    ...
    struct sockaddr *address, socklen_t *address_len)
{
  ...
  *address_len = sizeof(struct sockaddr);
  ...
  sprintf(c->sock_table[connfd]->sockname_cs, "%s.%s.cs.fifo", a, p);
  mkfifo(c->sock_table[connfd]->sockname_cs, 0666);
  sprintf(c->sock_table[connfd]->sockname_sc, "%s.%s.sc.fifo", a, p);
